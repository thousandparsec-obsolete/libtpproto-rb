...
def ok!(msg=nil)
  @conn.okay(msg || 'Manual okay')
end

def fail!(code)
  msg=nil
  code = TPProto::Fail::Code.const_get(code) if code.is_a? Symbol
  $conn.fail(code, msg || 'Manual failure')
end

def connect!(client=nil)
  ident = 'libtpproto-rb'
  ident = "#{client} [#{ident}]" if client
  $conn.connect!(ident)
end
